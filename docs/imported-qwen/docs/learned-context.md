...
- **Upstream Candidate Caches**: `~/.cursor/plugins/cache/.../superpowers/` and `~/.codex/.tmp/plugins/.../superpowers/` may contain fresher upstream plugin content. Compare them, promote validated improvements into canonical `/Users/steven/supremepowers/skills/`, then propagate adapters.
...
- Standardized superpowers source references to `/Users/steven/supremepowers/skills/` in active system docs.
- Added compatibility pointer file `/Users/steven/supremepowers/docs/imported-qwen/Learning_Explanatory_Styles.md` so older references resolve while `learning-output-style/latest/` and `explanatory-output-style/latest/` remain canonical style locations.
...
- Added `docs/SOURCE_OF_TRUTH_AND_LAYERS.md` to formalize authoring boundaries:
  canonical (`/Users/steven/supremepowers/skills/`) vs adaptation (`~/.qwen/...`) vs runtime hooks/plugins (`~/.claude/plugins/...`, `~/.qwen/hookify/`) vs archives (`~/.codex/archives/...`).
...
- Create `/Users/steven/supremepowers/docs/imported-qwen/docs/conversation-link-map.md` — neural-net reference for concept relationships
- `/Users/steven/supremepowers/docs/imported-qwen/QWEN_INVOKE_INDEX_BY_TYPE.md` is the canonical type-based roster; do not create a duplicate under `docs/`.
- Canonical use-cases doc path is `/Users/steven/supremepowers/docs/imported-qwen/Use_Cases_And_Triggers.md` (do not create a duplicate under `docs/`).
