...
- **Qwen-native standards import/adaptation** — Imported external reference docs into `/Users/steven/supremepowers/docs/imported-qwen/docs/` as `LINK_PATHS_USAGE_WORKFLOWS_AND_MULTI_ABILITY.md` and `LANGUAGE_AND_REFERENCE_STANDARD.md`, and updated `QWEN_INVOKE_INDEX_BY_TYPE.md` to Qwen-local canonical authoring language.
...
- **`using-superpowers` adapter review** — Compared `~/iterm2`, `~/.supremepower`, and `~/.qwen` copies; clarified that `/Users/steven/supremepowers/skills/` is the canonical authoring path while `~/.supremepower/skills/` is a host adapter.
- **Cross-root `using-superpowers` promotion** — Extended review to `~/.gemini`, `~/.cursor`, and `~/.codex`; promoted newer upstream candidate content from Cursor/Codex plugin caches into `/Users/steven/supremepowers/skills/using-superpowers/`.
...
- **Import provenance alignment** — Updated `IMPORT_MANIFEST.md` so superpowers import provenance points to canonical authoring at `/Users/steven/supremepowers/skills/` rather than old `~/.supremepower` wording.
...
- **Use-cases path normalization** — Confirmed canonical path `/Users/steven/supremepowers/docs/imported-qwen/Use_Cases_And_Triggers.md` and removed docs-path duplication intent from learned context.
- **Source of truth architecture** — Added `docs/SOURCE_OF_TRUTH_AND_LAYERS.md` defining canonical authoring (`/Users/steven/supremepowers/skills/`), adaptation layers in `.qwen`, runtime hook/plugin layers in `.claude`/hookify, and archive boundaries in `.codex/archives`.
...
- **Canonical source reference normalization (follow-up pass)** — Updated residual references in:
  ...
  to consistently reference `/Users/steven/supremepowers/skills/` as canonical propagation source.
...
- **Canonical Superpowers** — Cross-referenced with `/Users/steven/supremepowers/`
